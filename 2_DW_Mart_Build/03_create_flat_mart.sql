...
DROP SCHEMA IF EXISTS flat_mart CASCADE; -- CASCADE drops all the tables inside the schema too!
...
--CREATE OR REPLACE TABLE flat_mart.job_postings AS -- actually for the unique flat table we can use only this one line instead of all the rows above
-- we don't need a PK, and all the data types will be inherited from the source tables
-- but for the enterprise standards it's better to write the whole query, not the simplified one
SELECT
    -- Fact table fields
    jpf.job_id,
    jpf.job_title_short,
